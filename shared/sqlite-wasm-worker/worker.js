/*
 * Minimal Web Worker for androidx WebWorkerSQLiteDriver.
 * MVP: always open an in-memory SQLite DB (refresh clears data; no OPFS).
 * Protocol matches AndroidX sqlite-web-worker-test worker.
 */
...
function openRequest(id, requestData) {
    try {
        ...
        // 内存库：刷新页面即丢失，符合当前 Web MVP
        const newDatabase = new sqlite3.oo1.DB(':memory:');
